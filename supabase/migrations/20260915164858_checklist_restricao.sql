...
--
-- Rollback:
--   drop table public.restricao_tarefas;
--   drop function app.antes_de_gravar_tarefa();

create table public.restricao_tarefas (
...
create index on public.restricao_tarefas (restricao_id, criado_em);
create index on public.restricao_tarefas (criado_por);
create index on public.restricao_tarefas (concluida_por);

create or replace function app.antes_de_gravar_tarefa()
returns trigger
language plpgsql
set search_path = ''
as $$
...
create trigger antes_de_gravar_tarefa
  before insert or update on public.restricao_tarefas
  for each row execute function app.antes_de_gravar_tarefa();

alter table public.restricao_tarefas enable row level security;

-- Membro da obra da restrição faz tudo; ninguém de fora vê nem escreve.
create policy tarefas_select on public.restricao_tarefas
...
create policy tarefas_insert on public.restricao_tarefas
...
create policy tarefas_update on public.restricao_tarefas
...
create policy tarefas_delete on public.restricao_tarefas
